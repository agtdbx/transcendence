...
async def create_local_game_room(my_id : int,
    ...
    if game_room == None:
        print("\nWS : User", my_id, "already have a local game room", file=sys.stderr)
        return
    ...
    str_msg = create_game_room_status_message("createLocalRoomInfo")
...
async def local_game_room_add_something(my_id:int,
    ...
    if id_to_add == -1:
        ...
        print("\nWS : User", my_id, "Player add to team", target_team, file=sys.stderr)
...
async def local_game_room_remove_something(my_id:int,
    ...
    if id_to_remove == IA_ID:
        ...
        print("\nWS : User", my_id, "Player remove from team", target_team, file=sys.stderr)
    str_msg = create_game_room_status_message("updateLocalRoomInfo", game_room)
    await send_msg_to_id(my_id, connected_users, str_msg)
...
async def local_game_room_start_game(my_id:int,
    ...
    ret = await create_local_game_room(in_game_list, game_room["map_id"],
