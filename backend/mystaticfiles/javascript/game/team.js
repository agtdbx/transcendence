...
class Team {
	def constructor ( numberOfPlayers:int, team:int) -> None:
	...
	def tick( delta:float, paddlesKeyState:list, updateTime:bool) -> None:
		// Check input
		for i in range(len(this.paddles)):
			if updateTime:
	...
	def draw( win, powerUpEnable):
		for p in this.paddles:
			p.draw(win)
	...
	def applyPowerUpToPaddles( powerUp):
		for p in this.paddles:
